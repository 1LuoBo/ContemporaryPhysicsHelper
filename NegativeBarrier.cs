...
{
    [CustomEntity("ContemporaryPhysicsHelper/NegativeBarrier")]
    [Tracked]
    public class NegativeBarrier : Solid
    {
        private static bool disableRefill;

        private readonly List<NegativeBarrier> adjacent = new List<NegativeBarrier>();
        ...
        private NegativeBarrier(Vector2 position, float width, float height)
            : base(position, width, height, false)
        {
            ...
            Add(new HoldableCollider(OnNegativeQuantum));
        }

        public NegativeBarrier(EntityData data, Vector2 offset)
            : this(data.Position + offset, data.Width, data.Height) { }

        private void OnNegativeQuantum(Holdable negativeQuantum)
        {
            OnReflect();
        }
        ...
        private void OnReflect()
        {
            ...
            foreach (NegativeBarrier seekerBarrier in adjacent)
            {
        ...
        {
            Color backgroundColor = Calc.HexToColor("BBBCBB");
        ...
        private static void AddCollideCheck(ILContext il)
        {
            ...
            Logger.Log("DJMapHelper/NegativeBarrier",
        ...
        {
            if (data.Hit is NegativeBarrier barrier)
            {
                barrier.OnReflect();
            }
        }

        //private static void NegativeQuantumOnUpdate(On.Celeste.NegativeQuantum.orig_Update orig, NegativeQuantum self)
        //{
        //    List<Entity> NegativeBarrier = self.Scene.Tracker.GetEntities<NegativeBarrier>().ToList();
        ...
        {
            List<Entity> NegativeBarrier = self.Scene.Tracker.GetEntities<NegativeBarrier>().ToList();
            if (self.Holding?.Entity is NegativeQuantum)
            {
                NegativeBarrier.ForEach(entity => entity.Collidable = true);
        ...
        {
            List<Entity> barriers = self.Scene.Tracker.GetEntities<NegativeBarrier>();
        ...
        {
            NegativeBarrier barrier = player.CollideFirstOutside<NegativeBarrier>(player.Position + direction);
            if (barrier == null)
            {
                barrier = player.CollideFirst<NegativeBarrier>();
        ...
        {
            var NegativeBarrier = self.Scene.Tracker.GetEntities<NegativeBarrier>().ToList();
            NegativeBarrier.ForEach(entity => entity.Collidable = true);
            var collide = self.CollideCheck<NegativeBarrier>();
            NegativeBarrier.ForEach(entity => entity.Collidable = false);

            if (collide && pickup.Entity is NegativeQuantum)
            {
                return false;
            }
