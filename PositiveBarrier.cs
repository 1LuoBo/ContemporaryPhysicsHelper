...
{
    [CustomEntity("ContemporaryPhysicsHelper/PositiveBarrier")]
    [Tracked]
    public class PositiveBarrier : Solid
    {
        // Token: 0x06000F4A RID: 3914 RVA: 0x00036F94 File Offset: 0x00035194
        private static bool disableRefill;

        private readonly List<PositiveBarrier> adjacent = new List<PositiveBarrier>();
        ...
        private PositiveBarrier(Vector2 position, float width, float height)
            : base(position, width, height, false)
        {
            ...
            Add(new HoldableCollider(OnPositiveQuantum));
        }

        public PositiveBarrier(EntityData data, Vector2 offset)
            : this(data.Position + offset, data.Width, data.Height) { }

        private void OnPositiveQuantum(Holdable positiveQuantum)
        {
            OnReflect();
        }
        ...
        private void OnReflect()
        {
            ...
            foreach (PositiveBarrier seekerBarrier in adjacent)
            {
        ...
        {
            Color backgroundColor = Calc.HexToColor("2A2A2A");
        ...
        private static void AddCollideCheck(ILContext il)
        {
            ...
            Logger.Log("DJMapHelper/PositiveBarrier",
        ...
        {
            if (data.Hit is PositiveBarrier barrier)
            {
                barrier.OnReflect();
            }
        }

        //private static void PositiveQuantumOnUpdate(On.Celeste.PositiveQuantum.orig_Update orig, PositiveQuantum self)
        //{
        //    List<Entity> PositiveBarrier = self.Scene.Tracker.GetEntities<PositiveBarrier>().ToList();
        ...
        {
            List<Entity> PositiveBarrier = self.Scene.Tracker.GetEntities<PositiveBarrier>().ToList();
            if (self.Holding?.Entity is PositiveQuantum)
            {
                PositiveBarrier.ForEach(entity => entity.Collidable = true);
        ...
        {
            List<Entity> barriers = self.Scene.Tracker.GetEntities<PositiveBarrier>();
        ...
        {
            PositiveBarrier barrier = player.CollideFirstOutside<PositiveBarrier>(player.Position + direction);
            if (barrier == null)
            {
                barrier = player.CollideFirst<PositiveBarrier>();
        ...
        {
            var PositiveBarrier = self.Scene.Tracker.GetEntities<PositiveBarrier>().ToList();
            PositiveBarrier.ForEach(entity => entity.Collidable = true);
            var collide = self.CollideCheck<PositiveBarrier>();
            PositiveBarrier.ForEach(entity => entity.Collidable = false);

            if (collide && pickup.Entity is PositiveQuantum)
            {
                return false;
            }
